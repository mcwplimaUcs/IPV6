...
def ipv6Header(data):
    #data - array em bytes tamanho de 40 bytes
    #[0] I - unsigned int - 4 bytes [Version, Traffic Class, Flow Label]
    #[1] H - unsigned short - 2 bytes [Payload Length]
    #[2] B - unsigned char - 1 byte [Next Header]
    #[3] B - unsigned char - 1 byte [Hop Limit]
    packet = struct.unpack(">IHBB", data[0:8])
    
    srcAddress = socket.inet_ntop(socket.AF_INET6, data[8:24])
    dstAddress = socket.inet_ntop(socket.AF_INET6, data[24:40])

    version = packet[0] >> 28
    traffic_class = packet[0] >> 16
    traffic_class = int(traffic_class) & 4095
    flow_label = int(packet[0]) & 65535
    ...
    print "\tTraffic class: " + str(hex(traffic_class))
    print "\tFlow label: " + str(hex(flow_label))
    print "\tPayload Length: " + str(payloadLength)
    print "\tNext Header: " + str(nextHeader)
    print "\tHop limit: " + str(hopLimit)
    print "\tSource Address: "+ str(srcAddress) 
    print "\tDestination Address: "+ str(dstAddress) 

    data = data[40:]
    return data, nextHeader
